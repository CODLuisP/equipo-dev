...
import {
  ...
  Lock, ShieldCheck, File, Calendar, User, Info
} from "lucide-react";
...
function getTypeInfo(type: string) {
  ...
  return { icon: <File size={36} />, accent: '#8A9099', label: 'Archivo', bg: 'rgba(138,144,153,0.1)' };
}
...
function ShareView() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/shortlink?code=${code}`)
      .then(r => r.ok ? r.json() : Promise.reject())
      .then((data: SharePayload) => {
        const now = Date.now();
  ...
  return (
    <div className="share-page">
      ...
      <div className="share-container">
        {state === 'loading' && <div className="state-loading"><div className="spinner"></div></div>}
          ...
          return (
            <div className="main-card" style={{ '--accent': accent } as any}>
              <div className="card-header">
                <div className="main-icon" style={{ background: bg }}>{icon}</div>
                <h1>{payload.name}</h1>
      ...
      <style>{`
        .share-page { min-height:100vh; background:#05070A; color:#fff; display:flex; flex-direction:column; align-items:center; justify-content:center; font-family:'Inter',sans-serif; padding:24px; position:relative; overflow:hidden; }
        ...
        .share-container { width:100%; max-width:480px; margin:0 auto; position:relative; z-index:10; }
        ...
        .main-card { background:rgba(22,25,41,0.85); backdrop-filter:blur(50px); border:1px solid rgba(255,255,255,0.06); border-radius:40px; overflow:hidden; box-shadow:0 40px 100px rgba(0,0,0,0.8); }
        ...
        .info-grid { display:grid; grid-template-columns:repeat(2,1fr); gap:24px; background:rgba(0,0,0,0.25); border:1px solid rgba(255,255,255,0.04); border-radius:24px; padding:28px; margin-bottom:32px; }
        ...
        .download-btn { width:100%; height:64px; background:var(--accent); border:none; border-radius:20px; color:#000; font-size:16px; font-weight:800; cursor:pointer; transition:all 0.3s; display:flex; align-items:center; justify-content:center; gap:12px; box-shadow:0 10px 30px rgba(0,0,0,0.3); }
        ...
        @media (max-width:600px) { .card-header { padding:40px 24px 20px; } .card-header h1 { font-size:22px; } .card-body { padding:0 24px 40px; } .info-grid { grid-template-columns:1fr; gap:20px; padding:24px; } .main-icon { width:72px; height:72px; } .download-btn { height:58px; font-size:15px; } }
...
export default function CompartirCodePage() {
  return (
